...
def catheterExtraction(filename, mask, threshold):
    ...
    cv2.imshow('contour_ori', contour)
    return contour
...
def saveData(filename):    
    ...
    cv2.imshow('original', insert)

    for i in points:
        insert[i[0]][i[1]] = [0,255,255]
        insert[i[0]-1][i[1]-1] = [0,255,255]
        insert[i[0]+1][i[1]+1] = [0,255,255]
        insert[i[0]-1][i[1]-1] = [0,255,255]
        insert[i[0]+1][i[1]+1] = [0,255,255]
...
def curvature(originalPoint):
    ...
    curvature = np.abs(y_sd)/ ((1 + y_fd**2))**1.5
    print(fitEqu(originalPoint[:,1]))
    print(curvature)
    


folderPath  = '/Users/yifan_li/Library/Mobile Documents/com~apple~CloudDocs/Documents/Code/Python/CV/data1/1644326236279109569.jpg'
x,y,image,c = saveData(folderPath)
